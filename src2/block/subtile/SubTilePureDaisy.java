 ...
 */
package com.yiwp.batmanplusplus.block.subtile;
...
public class SubTilePureDaisy extends SubTileEntity {
	...
	private static ItemStack istack;
	private static Block nblock;
	private static Block oblock;
	private static boolean nbool;
	private static boolean obool;
	...
	public void onUpdate() {
		...
		if(block != Blocks.air) {
			...
			if(item != null) {
				istack = new ItemStack(item, 1, supertile.getWorldObj().getBlockMetadata(coords.posX, coords.posY, coords.posZ));
				Block output = nbool ? nblock : obool ? oblock : null;
				if(output != null) {
	...
	public static void isOreDict(String entry, Block end) {
		for(ItemStack ostack : OreDictionary.getOres(entry)) {
			ItemStack cstack = ostack.copy();
			if(cstack.getItemDamage() == Short.MAX_VALUE)
				cstack.setItemDamage(istack.getItemDamage());

			if(istack.isItemEqual(cstack))
				obool = true;
		}
	...
	public static void isValid(ItemStack entry, Block end) {
			end = nblock;
		if(entry.getItemDamage() == Short.MAX_VALUE)
			entry.setItemDamage(istack.getItemDamage());
		
		if(istack.isItemEqual(entry)) {
			nbool = true;
		}
