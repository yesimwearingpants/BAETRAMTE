...
public class TileAnvil extends TilePylon implements ISparkAttachable {
...
/*	private static final String TAG_STAGE = "stage";
	...
	private static final int[][][] PYLON_LOCATIONS = new int[][][] {
		{ { -3, 1, 3 }, { -3, 1, -3 }, { 3, 1, 3 }, { 3, 1, -3 }, { -4, 1, 0 }, { 4, 1, 0 } },
		{ { 0, 1, 4 }, { 0, 1, -4 } }
	};
	...
	public void updateEntity() {
		if(getBlockMetadata() < PYLON_LOCATIONS.length)
			for(int[] pylon : PYLON_LOCATIONS[getBlockMetadata()]) {
				...
				if(tile != null && tile instanceof ExtTilePylon)
					((ExtTilePylon) tile).activated = false;
			}
		...
		case 2 : { // Get Mana
			for(int[] pylon : PYLON_LOCATIONS[getBlockMetadata()]) {
				ExtTilePylon pylonTile = (ExtTilePylon) worldObj.getTileEntity(xCoord + pylon[0], yCoord + pylon[1], zCoord + pylon[2]);
				if(pylonTile != null) {
			...
			if(manaRequired == -1) {
				...
				manaRequired = 0;
				for(int[] pylon : PYLON_LOCATIONS[getBlockMetadata()])
					((ExtTilePylon) worldObj.getTileEntity(xCoord + pylon[0], yCoord + pylon[1], zCoord + pylon[2])).activated = false;
